...
func NewMsvcX64Environ(msvcVersion, sdkVersion, targetA, targetB, host string, flags *Flags) string {
	base := []string{
		...
		"",
		`@if not "%UCRTVersion%" == "" @set "INCLUDE=%UniversalCRTSdkDir%include\%UCRTVersion%\shared;%UniversalCRTSdkDir%include\%UCRTVersion%\ucrt;%UniversalCRTSdkDir%include\%UCRTVersion%\um;%INCLUDE%"`,
		`@if not "%UCRTVersion%" == "" @set "LIB=%UniversalCRTSdkDir%lib\%UCRTVersion%\ucrt\` + targetA + `;` + `%UniversalCRTSdkDir%lib\%UCRTVersion%\um\` + targetA + `;%LIB%"`,
		"",
